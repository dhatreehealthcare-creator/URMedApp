...
test("workspace authorization requires both the exact role and active status", () => {
  for (const role of ["vendor", "customer", "admin", "delivery"]) {
    assert.equal(isWorkspaceRoleAuthorized({ role, status: "active" }, role), true);
    assert.equal(isWorkspaceRoleAuthorized({ role, status: "inactive" }, role), false);
  }
  assert.equal(isWorkspaceRoleAuthorized({ role: "customer", status: "active" }, "admin"), false);
  assert.equal(isWorkspaceRoleAuthorized({ role: "admin", status: "active" }, "vendor"), false);
...
test("the shared route gate mounts workspace content only after exact-role authorization", () => {
  ...
  assert.match(gate, /setProfile\(isWorkspaceRoleAuthorized\(nextProfile, role\) \? nextProfile : null\)/);
