...
type Purchase = { id: number; purchaseNumber: string; invoiceNumber: string; invoiceDate: string; subtotalPaise: number; taxPaise: number; totalPaise: number; paymentStatus: string; status: string; supplierName: string; lineCount: number };
...
export function ProcurementCenter({ mode }: { mode: "purchase" | "masters" }) {
  ...
  const savePurchase = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(`${data.purchaseNumber} posted. Batch stock and the double-entry purchase ledger are updated.`);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Purchase could not be posted"); }
    finally { setBusy(""); }
  };
  ...
  if (mode === "masters") return <div className="portal-stack procurement-live"><Notice error={error} message={message} /><div className="portal-split">
    <section className="portal-panel"><div className="portal-panel-heading compact"><div><span className="portal-kicker">STOCKIST MASTER</span><h2>Suppliers</h2><p>Suppliers are private to this pharmacy and validated before posting purchases.</p></div><Building2 size={21} /></div>
