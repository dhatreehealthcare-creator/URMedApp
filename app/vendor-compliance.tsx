...
type BankAccount = { id: number; vendorId: number; businessName: string; bankName: string; accountName: string; accountLast4: string; ifscCode: string; verificationStatus: string; submittedAt: string };
type Payload = { applications: Application[]; licences: Licence[]; pharmacists: Pharmacist[]; bankAccounts: BankAccount[]; error?: string };
...
export function VendorCompliance() {
  ...
  const decide = async (entity: "licence" | "pharmacist" | "bank_account", id: number, decision: "verified" | "rejected") => {
  ...
  return <div className="portal-stack">
    ...
    {data && <section className="portal-panel"><div className="portal-panel-heading compact"><div><h2>Settlement bank accounts</h2><p>Review only masked account identity. Encrypted account numbers are never returned to the browser.</p></div></div><div className="compliance-review-list">{data.bankAccounts.map((item) => <article key={item.id}><div className="review-heading"><div><strong>{item.businessName} · {item.bankName}</strong><small>{item.accountName} · •••• {item.accountLast4} · {item.ifscCode}</small></div><ReviewStatus value={item.verificationStatus} /></div>{item.verificationStatus === "pending" && <><textarea onChange={(event) => setReasons((current) => ({ ...current, [`bank_account-${item.id}`]: event.target.value }))} placeholder="Reason required only when rejecting" rows={2} value={reasons[`bank_account-${item.id}`] || ""} /><div className="review-actions"><button disabled={busy === `bank_account-${item.id}`} onClick={() => void decide("bank_account", item.id, "verified")} type="button"><CheckCircle2 size={15} /> Verify</button><button className="reject" disabled={busy === `bank_account-${item.id}`} onClick={() => void decide("bank_account", item.id, "rejected")} type="button"><XCircle size={15} /> Reject</button></div></>}</article>)}</div></section>}
  </div>;
}
