...
export async function POST(request: Request) {
  try {
    ...
    if (order.paymentMethod !== "online") return privateJson({ error: "This is a cash-on-delivery order" }, { status: 409 });
    if (order.orderStatus === "cancelled") return privateJson({ error: "A cancelled order cannot be paid" }, { status: 409 });
    if (!["not_required", "approved"].includes(order.prescriptionStatus)) return privateJson({ error: "Payment opens after pharmacist prescription approval" }, { status: 409 });
    try {
    ...
    if (order.razorpayOrderId) return privateJson({ id: order.razorpayOrderId, amount: order.totalPaise, currency: "INR", keyId });
    ...
    if (!response.ok || !payload.id) {
      ...
      return privateJson({ error: payload.error?.description || "Razorpay could not create the payment" }, { status: 502 });
    }
    if (payload.amount !== order.totalPaise || payload.currency !== "INR" || payload.receipt !== order.orderNumber) {
      return privateJson({ error: "Razorpay returned an inconsistent payment order" }, { status: 502 });
    }
    await db.prepare("UPDATE orders SET razorpay_order_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(payload.id, order.id).run();
    return privateJson({ id: payload.id, amount: order.totalPaise, currency: "INR", keyId });
  } catch (error) {
